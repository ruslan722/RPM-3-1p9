...
class Student():
    ...
    # Свойство для доступа к возрасту студента
    @property
    def age(self):
        return self.__age
    ...
    def add_grade(self, subject, grade):
        if subject in self.__subjects:  # Если предмет есть у студента
            if subject not in self.__grades:
                self.__grades[subject] = []  # Если предмета нет в списке оценок, создаем его
            self.__grades[subject].append(grade)  # Добавляем оценку по предмету
        else:
            print(f"{subject} не является предметом у студента {self.__name}. Доступные предметы: {', '.join(self.__subjects)}")
    ...
    def __str__(self):
        ...
        return f"Имя: {self.__name}, Возраст: {self.__age}, Средняя оценка: {avg_grade:.2f}, Предметы с оценками: {graded_subjects_str}"
